...
One of the variables in the gravity model is the friction factor, which can assume various functional forms.  This analysis uses an inverse costs function, which pertains to the cost of travel between zones (or in this case, between stations).  \citep{Meyer2000}. A constant (b) is used which is empirically calculated based on the specific transit network. A b value of one was used for this analysis, as not enough information was provided to identify an alternate, more appropriate value.   
